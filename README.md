...
[🇬🇧 English](#beebo-a-interpreter-in-ocaml)

# Beebo — Интерпретатор на OCaml

Beebo (`.bbo`) — это простой процедурный язык программирования с компилятором-интерпретатором на основе управляющих таблиц, написанный на OCaml. Он включает в себя **лексический анализатор на базе конечного автомата**, **предсказывающий LL(1)-парсер** с использованием **таблицы синтаксического анализа** и **интерпретатор OPS на базе стековой машины**.
...
```bash
./src/beebo examples/demo.bbo
./src/beebo --tokens examples/demo.bbo # показывает лексемы
./src/beebo --ops examples/demo.bbo # показывает сгенерированный OPS
...
> Загляните в папку `docs/`, чтобы ознакомиться с подробной спецификацией языка, обзором архитектуры и примечаниями к реализации.
...
Объявление типов не требуется. Переменные динамически хранят целые числа, вещественные числа, строки или схемы.
...
Операторы со стандартным приоритетом: `*`, `/` (высший) > `+`, `-` (низший). Скобки изменяют приоритет. Деление всегда возвращает **вещественный** результат (real).
...
- Строковые литералы выделяются двойными кавычками. Экранированные последовательности: `\n` (перевод строки), `\\` (обратный слэш), `\"` (кавычка), `\t` (табуляция).
- Строки можно объединять (конкатенировать) с помощью оператора `+`. При конкатенировании числа автоматически преобразуются в строки.
...
Инструкция `output` выводит значение без добавления новой строки. Используйте `\n` в строках для переноса строки:

```
output "Hello\n"          // выводит "Hello", затем новую строку
output x                  // выводит значение x
output "x = " + x + "\n"  // объединенный вывод с новой строкой

```

### Ввод

Инструкция `input` читает строку и пытается сопоставить её с типами в следующем порядке: **integer** (целое число) → **real** (вещественное число) → **string** (строка, как запасной вариант).

```
input x                   // читает значение в переменную
input arr[0]              // читает значение в элемент массива

```

### Точка с запятой

Точки с запятой являются **разделителями инструкций**. Размещайте `;` между инструкциями внутри одного блока. Перед закрывающей скобкой `}` или концом файла (EOF) точку с запятой ставить **не нужно**.

```
x = 5;
y = 10;
output x + y // последняя инструкция, без ;

```

После блока (`}`), используйте `;` перед следующей инструкцией:
...
output "Done\n"        // ; отделяет блок от следующей инструкции

```

### Массивы

Одномерные (1D) и двумерные (2D) динамические массивы. Выделение памяти под массивы происходит автоматически при первом присваивании; явное объявление вида `name[size]` необязательно.

**Одномерные массивы (1D):**

```
arr[5]                    // объявление массива размером 5
arr[n]                    // объявление массива размером n
arr[0] = 42               // присваивание значения элементу
x = arr[3]                // чтение элемента

```

**Двумерные массивы (2D матрицы):**

```
mat[3][4]                 // объявление матрицы 3x4 (необязательно)
mat[0][0] = 10            // присваивание значения элементу (память выделяется автоматически)
mat[1][2] = 20
x = mat[0][0]             // чтение элемента

```

### Схемы (подобие структур)

Определение структурированных данных с именованными полями:
...
p = Point{x: 10, y: 20}; // создание экземпляра
output p.x;              // доступ к полю → 10
p.x = 42;                // присваивание значения полю
output p.y;              // → 20
...
- **Доступ**: `obj.field` в выражениях
- **Присваивание**: `obj.field = value`

Поля разделяются точкой с запятой в определении и запятыми в литералах.
...
Составные условия с операторами `&&` (И), `||` (ИЛИ), `!` (НЕ):
...
Функции могут принимать 0 или более параметров. Инструкция `return` должна быть указана явно. Функции могут вызывать сами себя рекурсивно.

Вызов функций как отдельных инструкций (игнорируя возвращаемое значение):

```
greet("Beebo") // вызов без использования возвращаемого значения
...
- `string(x)` — преобразование любого значения в строку
- `real(x)` — преобразование в вещественное число (из int или string)
- `integer(x)` — преобразование в целое число (из real или string)
...
- `sleep(ms)` — пауза на `ms` миллисекунд
- `clear_screen()` — очистка терминала (ANSI `\033[2J\033[H`)
- `move_cursor(x, y)` — перемещение курсора в столбец `x` и строку `y` (индексация с 1)
- `get_key()` — неблокирующее чтение одного нажатия клавиши; возвращает пустую строку, если ни одна клавиша не нажата
- `cursor_hide()` / `cursor_show()` — скрыть/показать курсор терминала
- `set_color(fg)` / `set_color(fg, bg)` — установить цвет текста / оба цвета (0–7)
- `reset_color()` — сброс цвета к значениям по умолчанию
- `term_width()` / `term_height()` — размеры терминала (на основе переменных `$COLUMNS`/`$LINES`)
...
Компилятор-интерпретатор следует классической модели **анализа и синтеза** с тремя фазами, управляемыми таблицами:

### 1. Лексический анализатор (`lexer.ml`)

Конечный автомат с явной таблицей переходов `(state × character_class) → next_state`.

- 10 состояний: START, ID, NUM, REAL_FRAC, STRING, OPERATOR, COMMENT и т. д.
- Классы символов: LETTER, DIGIT, DOT, QUOTE, COLON, операторы, разделители
- Ключевые слова определяются с помощью таблицы поиска после токенизации идентификаторов
- Экранированные последовательности (`\n`, `\\`, `\"`, `\t`) обрабатываются в строковых литералах

### 2. Парсер и генератор OPS (`grammar.ml`, `parser.ml`)

- **Грамматика**: 102 продукции в **нестрогой нормальной форме Грейбах**
- **Таблица разбора**: `(nonterminal × terminal) → production` с использованием предсказывающего LL(1)-анализа
- **Автомат с магазинной памятью**: МП-автомат, использующий стек задач (задачи MATCH, PARSE, ACTION)
- **Генерация OPS**: Семантические действия, привязанные к каждой продукции, создают постфиксные инструкции

### 3. Интерпретатор OPS (`interpreter.ml`)

Стековая машина, выполняющая инструкции OPS:
...
- **Логика**: AND, OR, NOT
- **Управление потоком**: JMP, JMPF, LABEL
- **Функции**: FUNC_ENTRY, CALL_USER, RET, ARG
- **Ввод-вывод**: INPUT_STR, OUTPUT
- **Математика**: CALL (sqrt, exp, log, sin, cos, abs)
- **Terminal**: резервный вызов CALL_USER (sleep, get_key, clear_screen, move_cursor и т. д.)
...
| ------------------------ | ------------------------------------------ |
| `V_INT(n)`               | Целое число                                |
| `V_REAL(r)`              | Веществ. число (float)                     |
| `V_STR(s)`               | Строка                                     |
| `V_ARR(arr)`             | Одномерный динамический массив             |
| `V_MAT(arr)`             | Двумерная матрица                          |
| `V_SCHEMA(name, fields)` | Именованная структура с хэш-таблицей полей |

Арифметика: автоматическое приведение типов int → real, деление всегда возвращает real.
Строковый оператор `+`: автоматически преобразует нестроковые операнды в строку.
Модель памяти: хэш-таблица, сопоставляющая имена со значениями.

---

## Диагностика ошибок

Компилятор предоставляет сообщения об ошибках, содержащие:

- **Номер строки** с ошибкой
- **Номер столбца** ошибочного символа
...
```
beebo [options] <file.bbo>
--tokens Вывести список токенов после лексического анализа
--ops Вывести сгенерированный код OPS (польская нотация)
...
Изначальная грамматика содержала левую рекурсию в продукциях выражений. После её устранения и левой факторизации (нестрогая форма Грейбах):
...
Таблица разбора сопоставляет пары `(nonterminal, terminal) → production_id`:

```
add_set NT_STMT [T_KW_IF] 5 // Stmt → IfStmt при обнаружении 'if'
add_set NT_STMT [T_KW_FUNC] 72 // Stmt → FuncDef при обнаружении 'func'

```

### Соглашение о стеке OPS

**Первый помещенный элемент = низ стека (база/адрес), второй помещенный элемент = верх стека (значение/индекс)**.

- STORE: извлечь значение (верх), извлечь адрес → сохранить значение по адресу
- INDEX: извлечь индекс (верх), извлечь базу → положить в стек ADDR_ARR
- ALLOC: извлечь размер (верх), извлечь имя → выделить память под массив
- GET_FIELD: извлечь схему (верх), положить в стек значение поля
- SET_FIELD: извлечь значение (верх), извлечь схему → изменить поле, положить схему обратно в стек
- MAKE_SCHEMA: извлечь пары полей, извлечь имя типа → положить в стек V_SCHEMA
